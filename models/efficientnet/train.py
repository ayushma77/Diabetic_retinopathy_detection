...
# # Load configuration
# config = config_load("models/efficientnet/configs/config_efficientnet.yaml")
...
# model =  EfficientNet(num_classes=config['num_classes']).to(device)
...
# torch.save(model.state_dict(), f"models/efficientnet/{config['model']['name']}_model.pth")
#1.prepare datasets
import os
...
if __name__=="__main__":
    ...
    config = load_config("efficientnet")
    BATCH_SIZE=config['model']['args']['batch_size']
    num_workers=config['model']['args']['num_workers']
    #create folder
    ...
    model = EfficientNetModel(num_classes=config['model']['args']['num_classes']).to(device)
    #training
    LR=0.001
    EPOCHS=10
